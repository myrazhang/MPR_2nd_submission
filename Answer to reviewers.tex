...
\textbf{The benefits of using MP models to represent discrete event systems are many. As already mentioned, %above,
...
\textbf{The major concern about the application of MP is the computational complexity, %and the solving procedure
...
[2] An MP model was proposed to minimize the makespan of single-server manufacturing systems in \cite{di2020makespan}, %but this
while our work is more general and can model multi-server systems. 

[3] \cite{basile2012k} proposed the model based on PN, not TPN, because %it deals with 
...
it is used to bound the transition firing time. %The value of $M$ can be chosen as the upper bound of marking of place $p$ minus $(W^{p,t}-1)$ for constraints (B4). 
